...
async fn test_nfs_walkdir_performance() {
        ...
        match tokio::time::timeout(Duration::from_millis(100), rx.recv()).await {
            Ok(Some(entry)) => {
                ...
                if total_entries % 100 == 1 {
                    ...
                    println!("┌──────┬────────────────────────┬──────────┬────────┬────────────┬─────────┬─────────────┐");
                    println!("│ {:<4} │ {:<24} │ {:<10} │ {:<6} │ {:<10} │ {:<7} │ {:<13} │", "类型", "文件名", "大小", "权限", "硬链接", "软连接", "修改时间");
                    println!("├──────┼────────────────────────┼──────────┼────────┼────────────┼─────────┼─────────────┤");
                }
                ...
                let format_time = |time: Option<i64>| -> String {
                    time.map(|ts| {
                        let system_time = std::time::SystemTime::UNIX_EPOCH + std::time::Duration::from_millis(ts as u64);
    ...
    if file_count > 1000 {
        ...
        if scan_speed < expected_min_speed {
            eprintln!("⚠️  扫描速度过低: {:.2} 文件/秒 < 期望 {:.2} 文件/秒", scan_speed, expected_min_speed);
        } else {
